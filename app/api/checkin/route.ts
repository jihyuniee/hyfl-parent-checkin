...
export async function POST(request: Request) {
  try {
    ...
    const db = getDb();
    const [checkinEvent] = await db.select().from(events)
      .where(and(eq(events.id, eventId), or(eq(events.status, "active"), eq(events.status, "scheduled"))))
      .limit(1);
    if (!checkinEvent || !checkinEvent.targetGrades.split(",").map(Number).includes(grade)) {
      return Response.json({ error: "현재 체크인 가능한 학년의 행사가 아닙니다." }, { status: 400 });
    }
    ...
    if (existing) {
      return Response.json(
        { error: "이미 참석 등록된 학생입니다. 인원 변경은 안내 담당자에게 말씀해 주세요." },
        { status: 409 },
      );
    }
